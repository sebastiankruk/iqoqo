...
export function FreshArrivals({ publicMode = false }: { publicMode?: boolean } = {}) {
  const t = useTranslations("FreshArrivals");
  ...
  if (isError) {
    return (
      <section aria-label={t("ariaLabel")}>
        <h2 className="mb-3 font-serif text-xl font-bold text-foreground">{t("title")}</h2>
    ...
    <section aria-label={t("ariaLabel")}>
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h2 className="font-serif text-xl font-bold text-foreground">{t("title")}</h2>
      ...
      {isLoading ? (
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          {items.map(item => {
            const coverUrl =
              getCoverUrl(item.cover_url ?? undefined) || (item.meta?.["cover_url"] as string | undefined);
              ...
              <Link href={`/manifestation/${item.id}`} key={item.id} className="group w-36 shrink-0 sm:w-40">
                <div className="relative mb-3 aspect-[2/3] overflow-hidden rounded-lg shadow-md bg-secondary transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-xl">
                  {(isProcessing || item.cover_status === "pending") && (
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-2 bg-background/60 backdrop-blur-sm p-4 text-center">
                      ...
                      <span className="text-xs font-medium text-foreground">
                        {item.cover_status === "pending" ? t("generating") : t("processing")}
                      </span>
                    </div>
                  )}
                  ...
                  {coverUrl ? (
                    <Image
                      src={coverUrl}
                      alt={`Cover of ${item.title ?? t("untitled")}`}
                      fill
                ...
                <h3 className="truncate text-sm font-semibold text-card-foreground">{item.title ?? t("untitled")}</h3>
                <p className="mt-0.5 flex items-center gap-1.5 truncate text-xs text-muted-foreground">
                  <span className="inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-accent/60" />
                  {item.authors?.[0] ?? t("unknown")}
                </p>
              </Link>
            );
          })}
