...
import type { WorkDetail, WorkDetailManifestation } from "@/types/frbr";
...
/**
 * Client component for rendering the detailed FRBR Work view.
 *
 * @param {WorkDetailClientProps} props - Component properties.
 * @returns {JSX.Element} Interactive Work detail UI.
 */
export function WorkDetailClient({ initialWork }: WorkDetailClientProps) {
  const router = useRouter();
  const work = initialWork;

  // Flatten all manifestations to find the primary cover
  const allManifestations: WorkDetailManifestation[] = [];
  for (const expr of work.expressions || []) {
    for (const m of expr.manifestations || []) {
      allManifestations.push(m);
    }
  }

  const primaryCoverManifestation = allManifestations.find(m => m.cover_url);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      >
              ...
              <div className="relative mx-auto h-64 w-48 shrink-0 overflow-hidden rounded-xl border border-border/60 bg-muted/40 shadow-md md:mx-0">
                {primaryCoverUrl ? (
                  ...
                  <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-muted-foreground/60">
                    ...
                    <span className="text-xs font-medium">Conceptual Work</span>
                  </div>
                )}
              </div>

              {/* Work Metadata Header */}
              <div className="flex flex-1 flex-col gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {allManifestations.length} {allManifestations.length === 1 ? "edition" : "editions"}
                  </span>
                </div>
                  ...
                  >
                    ...
                    {normalizeAuthors(work.authors).map((author, idx) => (
                      <span key={author} property="schema:name" itemProp="name">
                        ...
                        {idx < work.authors.length - 1 && <span className="text-muted-foreground/60">, </span>}
                      </span>
                    ))}
                  </div>
                )}

                {/* FRBR Hierarchy description */}
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                  This page represents the conceptual FRBR Work entity. All expressions (translations, performances,
                  revisions) and physical manifestations (books, records, audiobooks) are linked below.
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Expressions & Manifestations Section */}
        <div className="mx-auto max-w-6xl px-4 pt-10 sm:px-6">
          <div className="space-y-10">
            {work.expressions && work.expressions.length > 0 ? (
              work.expressions.map((expr, exprIdx) => {
                const exprTitle = `${expr.language ? expr.language.toUpperCase() + " " : ""}${expr.content_type || "Realization"}`;
                return (
                  <div
                    key={expr.id || exprIdx}
                    className="space-y-4 rounded-xl border border-border/60 bg-card p-6 shadow-sm"
                    itemProp="hasExpression"
                    vocab="http://iflastandards.info/ns/frbr/frbrer/"
                    typeof="Expression"
                    resource={`#expression-${expr.id}`}
                  >
                    {/* Expression Subheading */}
                        ...
                        expr.manifestations.map(m => {
                          ...
                          return (
                            <Link
                              ...
                              className="group flex gap-4 rounded-lg border border-border/40 bg-background/60 p-3 transition-all hover:border-primary/40 hover:bg-muted/40 hover:shadow-sm"
              ...
              <div className="rounded-xl border border-dashed border-border/80 p-8 text-center text-muted-foreground">
                ...
                <p>No expressions or editions found for this work.</p>
              </div>
            )}
          </div>
        </div>
