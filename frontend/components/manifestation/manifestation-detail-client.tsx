...
import { BookOpen, Loader2, Disc, ImagePlus } from "lucide-react";
...
export interface ManifestationDetailClientProps {
  manifestationId: number;
  initialManifestation?: CatalogEntry | null;
}
 ...
 *
 * @param {ManifestationDetailClientProps} props - Component props.
 * @returns {JSX.Element} Manifestation detail component.
 */
export function ManifestationDetailClient({ manifestationId, initialManifestation }: ManifestationDetailClientProps) {
  const t = useTranslations("Manifestation");
  const router = useRouter();

  const { data: userProfile } = useProfile();
  const { data: manifestation, isLoading, isError } = useManifestation(manifestationId, initialManifestation);
