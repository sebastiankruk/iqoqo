...
Testing:
- A unit test `tests/test_profile.py::test_user_to_dict_includes_avatar` asserts that `User.to_dict()` includes `avatar_url`.

Notes:
- If the database in your deployment does not yet have the `avatar_url` column, apply migrations (Alembic) to add it.
