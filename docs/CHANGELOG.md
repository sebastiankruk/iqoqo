...
- **SPARQL Query Endpoint**: Read-only SPARQL Protocol endpoint at `/api/sparql` supporting GET (`?query=...`) and POST (body) with content negotiation for SPARQL results JSON, SPARQL results XML, Turtle, JSON-LD, CSV, and TSV. Resource isolation via subprocess execution with configurable timeout (15s), graph size limits (5,000 items, 100,000 triples), concurrency limits (4 concurrent queries), and query size caps (10KB). Protected by `@require_auth` and `read:metadata` permission. Rate limited to 10 queries per minute.
- **Public Linked Data Endpoints**: Public RDF endpoints at `/api/public/works/<id>`, `/api/public/expressions/<id>`, `/api/public/manifestations/<id>`, and `/api/public/items/<id>` serving FRBR/Schema.org semantic metadata with content negotiation (JSON-LD, Turtle, N-Triples) via `Accept` header or `?format=` query parameter. Open CORS headers (`Access-Control-Allow-Origin: *`) for AI agents and Linked Data crawlers. HTML requests redirect (303) to the frontend page. Rate limited to 120 requests per minute per endpoint.
- **Schema.org SEO Mappings**: Structured Schema.org metadata embedded in RDF serializations for search engine optimization. Maps FRBR entities to Schema.org types (`Book`, `Audiobook`, `MusicAlbum`, `Movie`, `Game`, `MusicEvent`, `CreativeWork`, `Collection`) with properties (`isbn`, `publisher`, `inLanguage`, `datePublished`, `image`, `author`, `contributor`, `numberOfPlayers`). Enables rich snippets in Google, Bing, and Yandex search results.
- **Data Sovereignty Export**: Streaming export endpoint at `/api/items/export` supporting JSON-LD, Turtle, and JSON formats. Authenticated users can export their complete library in Linked Data or hierarchical JSON adhering to FRBR Group 1 ontology. Chunked streaming serialization prevents memory pressure for large collections. Includes canonical JSON-LD `@context` with FRBR, Schema.org, Dublin Core, and iqoqo ontology namespaces.
- **Canonical Linked Open Data IRI Minting**: Implemented `iri` property on core FRBR models (`Work`, `Expression`, `Manifestation`, `Item`) minting canonical IRIs using `BASE_URL` environment variable (default: `https://iqoqo.cc`). Each entity receives a stable dereferenceable IRI (e.g., `https://iqoqo.cc/works/42`).
- **FRBR ETL Scripts**: Operational Makefile targets for FRBR data curation: `make audit-frbr` (integrity audit via `scripts/audit_frbr_integrity.py`, supports `ARGS="--json --verbose"`), `make etl-frbr` (strict cleanup via `scripts/etl_frbr_strict.py`, supports `ARGS="--dry-run --verbose"`), and `make sync-ontology` (ontology sync check via `scripts/sync_ontology.py`, supports `ARGS="--check"`). All targets support `USE_DOCKER=true` for production container execution.
...
- **mykg OpenCode Agent Harness**: Added sandboxed Docker harness for running the opencode CLI as an alternative LLM agent backend for mykg knowledge-graph indexing. New `opencode_daemon.py` mirrors `agy_daemon.py` with opencode-specific CLI invocation (`opencode run --auto -m <model> --variant <variant>`), credential bootstrap, and prompt sanitization. New `mykg-opencode-daemon` Docker Compose service with identical hardening (read-only rootfs, dropped caps, non-root user, resource limits). Agent-aware egress filtering via `AI_AGENT` env var selects between `allowlist.conf` (agy/Google) and `allowlist-opencode.conf` (opencode Go). Makefile `AI_AGENT` selector with per-agent default models (`agy`: `gemini-3.8-flash-low`, `opencode`: `opencode-go/muse-spark-1.3-contributor`) and minimal effort defaults. New `agent-opencode` profile in `mykg_config.yaml`.

### Changed

- **FRBR F3 Manifestation Column Promotion** ⚠️ **BREAKING**: Promoted core physical attributes (`isbn13`, `publisher`, `format_type`) from unstructured JSONB `meta` to indexed typed relational columns on `Manifestation` with linear Alembic migration `v0_7_19_f3_column_promotion`, data backfilling, metadata key pruning, and backward-compatible API serialization. Custom SQL queries referencing `meta->>'isbn13'` should be updated to use the `isbn13` column directly. See [UPGRADE_0.8.0.md](UPGRADE_0.8.0.md) for migration details.

### Fixed

- **XSS Prevention in Public Endpoints** (#295): Sanitized all user-controlled strings in public profile and shared collection responses to prevent reflected cross-site scripting via `bio`, `display_name`, and collection metadata fields.
- **SSRF Redirect Safety**: Hardened `safe_get` in `app/utils/covers.py` and `app/utils/http_client.py` with strict URL type coercion on redirect `Location` headers to prevent crashes and SSRF bypasses via non-string redirect targets.
- **Host Header Poisoning Defense**: Added strict domain validation and secure fallback hosts in frontend auth exchange routes (`frontend/app/api/auth-exchange/route.ts`) to prevent host poisoning and credential leakage via unvalidated `X-Forwarded-Host` headers.
...
- **AI Sandbox Egress Allowlist Hardening & Daemon Sanitization (Review 4 Finding SEC-01)**: Surgically scoped `deploy/sandbox_proxy/allowlist.conf` to 12 verified endpoints required for `agy` CLI operation, completely removing broad wildcard domains (`*.google.com`, `*.googleapis.com`, `*.googleusercontent.com`, `*.google`, `*.goog`). Documented requirement of `www.googleapis.com:443` strictly for `agy` startup OAuth2 UserInfo token validation and `play.googleapis.com:443` for Google Clearcut streaming telemetry. Added prompt payload sanitization (`sanitize_task_payload`) in `.agents/skills/iqoqo-mykg/scripts/agy_daemon.py` to redact `googleapis.com`, Drive, Docs, cloud URLs, base64 data URIs, and large unbroken binary blobs, alongside injected execution guardrail policies to block exfiltration vectors in depth.
