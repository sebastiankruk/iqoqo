...
- **AI Sandbox Egress Allowlist Hardening & Daemon Sanitization (Review 4 Finding SEC-01)**: Surgically scoped `deploy/sandbox_proxy/allowlist.conf` to 11 verified endpoints required for `agy` CLI operation, completely removing broad wildcard domains (`*.google.com`, `*.googleapis.com`, `*.googleusercontent.com`, `*.google`, `*.goog`) and dropping unnecessary subdomains like `play.googleapis.com`. Documented requirement of `www.googleapis.com:443` strictly for `agy` startup OAuth2 UserInfo token validation. Added prompt payload sanitization (`sanitize_task_payload`) in `.agents/skills/iqoqo-mykg/scripts/agy_daemon.py` to redact `googleapis.com`, Drive, Docs, and cloud URLs, alongside injected execution guardrail policies to block exfiltration vectors in depth.
