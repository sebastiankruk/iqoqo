...
def upgrade():
    ...
    if not inspector.has_table("scan_telemetry", schema="inventory"):
        op.create_table(
            "scan_telemetry",
    ...
    if inspector.has_table("llm_telemetry", schema="inventory"):
        with op.batch_alter_table("llm_telemetry", schema="inventory") as batch_op:
            if not has_column("llm_telemetry", "status", "inventory"):
                batch_op.add_column(sa.Column("status", sa.String(length=20), nullable=False, server_default="success"))
            if not has_column("llm_telemetry", "error_message", "inventory"):
