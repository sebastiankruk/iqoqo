...
def upgrade():
    ...
    op.create_table('feedback_comments',
        ...
        sa.ForeignKeyConstraint(['feedback_item_id'], ['social.feedback_items.id' if is_pg else 'feedback_items.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['user_id'], ['auth.users.id' if is_pg else 'users.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        schema='social' if is_pg else None
    )
    op.create_index(op.f('ix_social_feedback_comments_feedback_item_id'), 'feedback_comments', ['feedback_item_id'], unique=False, schema='social' if is_pg else None)
    op.create_index(op.f('ix_social_feedback_comments_user_id'), 'feedback_comments', ['user_id'], unique=False, schema='social' if is_pg else None)
    ...
    with op.batch_alter_table('feedback_items', schema='social' if is_pg else None) as batch_op:
...
def downgrade():
    ...
    with op.batch_alter_table('feedback_items', schema='social' if is_pg else None) as batch_op:
        batch_op.add_column(sa.Column('comments', postgresql.JSONB() if is_pg else sa.JSON(), server_default='[]', nullable=False))
    ...
    op.drop_index(op.f('ix_social_feedback_comments_user_id'), table_name='feedback_comments', schema='social' if is_pg else None)
    op.drop_index(op.f('ix_social_feedback_comments_feedback_item_id'), table_name='feedback_comments', schema='social' if is_pg else None)
    op.drop_table('feedback_comments', schema='social' if is_pg else None)
