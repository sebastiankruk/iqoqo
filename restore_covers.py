...
def restore_covers(zip_path):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if os.path.exists(src_covers): shutil.copytree(src_covers, dst_covers, dirs_exist_ok=True)
        ...
        with app.app_context():
            for m_data in data.get("manifestations", []):
                if not m_data.get("cover_path"): continue
...
if __name__ == "__main__":
    ...
    restore_covers(parser.parse_args().backup_file)            print("✅ Restore complete.")
