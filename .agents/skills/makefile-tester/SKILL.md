...
1. **Pre-emptive Formatting:** BEFORE running any linters, ALWAYS run code formatters to fix trivial issues automatically using the virtual environment:
   - Run: `.venv/bin/python -m black app/ tests/ scripts/` (or `make format-python` after activating venv)
...
1. **Testing:** Run `make test` (or ensure it uses `.venv/bin/pytest`).
   - _Known Issue:_ You will see `ECONNREFUSED ::1:3000` AggregateErrors during frontend tests. Ignore these; they are expected when the Next.js dev server is not running. As long as the final test exit code is `0`, tests passed.
