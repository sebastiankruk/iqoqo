...
## Context
Project iqoqo uses a Makefile for testing and linting. Full runs (`make lint`, `make test`) can be time-consuming. You must optimize the feedback loop.

## Instructions

1. **Initial Run:** Execute the master command (`make lint` or `make test`).
2. **Identify Failures:** If the master command fails, identify which specific sub-task failed (e.g., `flake8`, `mypy`, `pytest` for backend, or `npm run lint`, `vitest` for frontend).
3. **Targeted Re-runs:** Fix the underlying code issue. Do **not** run the master command again yet. Instead, run the specific failing sub-task command directly to verify the fix quickly. Here are targeted make sub-tasks:
