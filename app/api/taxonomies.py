...
def make_taxonomies_cache_key() -> str:
    """Generate a deterministic cache key for taxonomies.
    ...
    user_id = getattr(g, "user_id", "anon") if scope == "user" else "global"
    parsed = urlparse(request.full_path)
    sorted_params = urlencode(sorted(parse_qsl(parsed.query)))
    return f"taxonomies:{user_id}:{parsed.path}?{sorted_params}"


def extract_taxonomies_data(
    scope: str = "global",
    ...
    """Extract distinct tags, collections, genres, publishers, and expression kinds."""
...
@cache.cached(timeout=3600, key_prefix=make_taxonomies_cache_key)  # type: ignore[arg-type]
def get_taxonomies() -> Response | tuple[Response, int]:
    ...
    try:
        ...
        logger.error("Error fetching taxonomies: %s", e)
...
def get_boardgame_mechanics() -> Response | tuple[Response, int]:
    ...
    try:
        ...
        logger.error("Error fetching board game mechanics: %s", e)
