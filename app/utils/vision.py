...
def extract_metadata_from_cover(image_bytes: bytes, mime_type: str = "image/jpeg", user_id: str | None = None) -> dict | None:
    ...
    try:
        ...
        if result:
            return result
    except (ValueError, KeyError, IndexError, AttributeError, TypeError, OSError, RuntimeError, Exception) as e:
...
def _extract_via_gemini(image_bytes: bytes, mime_type: str, user_id: str | None = None) -> dict | None:
    ...
    except (ValueError, KeyError, TypeError, OSError, RuntimeError, ConnectionError) as e:
