...
function _onBookPost(isAdd) {
  ...
  let metadata = ($('#lastISBN').hasClass('notfound') || !isAdd)
               ? {
                    "Title": $('#lastISBN .title .meta-value input').val(),
                    "Authors": $('#lastISBN .authors .meta-value input').val().split(',').map(t => t.trim())
                 }
               : {}
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      let message = (isAdd)
                  ? `Successfully added ${isbn} item`
                  : `Successfully updated ${isbn} manifestation`;
      showToast(message);
