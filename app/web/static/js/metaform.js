...
function setButtonPrimary(addButton) {
  let primaryButton = addButton ? 'add' : 'update';
  let secondaryButton = addButton ? 'update' : 'add';
...
function updateLabel(sisbn, data) {
  let isbn = sisbn.substr(sisbn.length - 1) === 'X' ? sisbn.substr(0, sisbn.length - 1) : sisbn;
...
function onMetaUpdate() {
  let empty = $('.meta-value input').val() === '';
  $('button.clear').toggleClass('can-clear', !empty);

  let isbnEmpty = $('.isbn .meta-value input').val() === '';
