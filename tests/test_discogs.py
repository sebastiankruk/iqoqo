...
def test_fetch_discogs_metadata_uses_consumer_key_auth():
    ...
    with patch.dict(
        os.environ,
        {"DISCOGS_CONSUMER_KEY": "test_key", "DISCOGS_CONSUMER_SECRET": "test_secret", "DISCOGS_USER_TOKEN": "should_be_ignored"},
    ):
...
def test_fetch_discogs_metadata_falls_back_to_legacy_token():
    ...
    with patch.dict(
        os.environ, {"DISCOGS_CONSUMER_KEY": "", "DISCOGS_CONSUMER_SECRET": "", "DISCOGS_USER_TOKEN": "test_token"}, clear=True
    ):
        with patch("app.utils.discogs.requests.get") as mock_get:
            mock_response = MagicMock()
            mock_response.status_code = 200
...
def test_fetch_discogs_metadata_returns_none_if_no_creds():
    ...
    with patch.dict(os.environ, {"DISCOGS_CONSUMER_KEY": "", "DISCOGS_CONSUMER_SECRET": "", "DISCOGS_USER_TOKEN": ""}, clear=True):
