...
@patch.dict(os.environ, {"TMDB_API_KEY": "dummy_key"})
@patch("app.utils.tmdb.requests.get")
def test_fetch_video_metadata_with_cast_and_director(mock_get):
    """Test that TMDB metadata includes cast and director information."""
    mock_resp = MagicMock()
    mock_resp.json.return_value = {
        "results": [
            {
    ...
    result = fetch_video_metadata("883929153526")
